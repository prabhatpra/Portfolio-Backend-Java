...
public class ContactServiceImpl implements ContactService {
    ...
    public ApiResponseDto contactUser(RequestDto request) {
        ...
        String name = normalize(request.getName(), "Name");
        String email = normalize(request.getEmail(), "Email").toLowerCase();
        String subject = normalize(request.getSubject(), "Subject");
        String message = normalize(request.getMessage(), "Message");
        ...
        // 5. Email send 
        emailService.sendContactMail(saved);
    ...
    public ApiResponseDto replyContact(Long id, ReplyRequestDto request) {
        ...
        Contact contact = getContact(id);
        String replyMessage = normalize(request.getReplyMessage(), "Reply message");
